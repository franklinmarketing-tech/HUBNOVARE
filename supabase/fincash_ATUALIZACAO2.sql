-- ============================================================================
-- FINCASH — ATUALIZAÇÃO 2 (subcategorias, orçamento fixo/variável, importação)
-- ============================================================================
-- Cole inteiro no SQL Editor do Supabase e execute de uma vez só.
--
-- ⚠️ RODE DEPOIS de `fincash_TUDO.sql`, que já foi executado em 09/09/2026.
-- Este arquivo é ADITIVO: acrescenta coluna e tabela, e NÃO apaga dado nenhum.
--
-- Junta dois arquivos, nesta ordem:
--   1. fincash_v3.sql          subcategorias (fin_categorias.pai_id), ordem das
--                              categorias, subcategoria opcional em lançamento e
--                              recorrência, e o orçamento em modo fixo/variável
--   2. fincash_importacao.sql  importação de extrato OFX, com a tabela de chaves
--                              que impede lançamento duplicado
--
-- O QUE MUDA EM TABELA QUE JÁ TEM DADO
--   • `fin_categorias` ganha `pai_id` e `ordem` — as duas opcionais
--   • `fin_lancamentos` e `fin_recorrencias` ganham `subcategoria_id` opcional:
--     lançamento sem subcategoria continua válido e continua somando no pai
--   • `fin_orcamentos.mes_ref` deixa de ser obrigatório (é assim que a linha
--     "fixa", que vale para todos os meses, passa a caber na mesma tabela)
--   • a restrição de nome único por usuário é trocada por nome único DENTRO do
--     pai, que é o que permite "Mercado" existir dentro de duas categorias
--
...
-- ####################################################################
-- ARQUIVO fincash_importacao.sql
-- ####################################################################
