// CÓPIA de novareapp/src/lib/lifeplan.ts.
// O Workspace não importa do novareapp: os projetos publicam por caminhos
// diferentes e nada de lá é refatorado a partir daqui. Se a conta mudar num
// lado, precisa ser conferida no outro.
...
export interface LifePlanInput {
  ...
  rentRealPct: number;       // % a.a. real (acima da inflação), BRUTA de IR
  /** Inflação anual esperada. Precisa existir porque o IR incide sobre o
   *  ganho NOMINAL, não sobre o real. Padrão: meta do Banco Central. */
  inflacaoPct?: number;
  /** Alíquota de IR sobre o rendimento. Padrão: 15%, a faixa de quem deixa
   *  o dinheiro aplicado por mais de dois anos — o caso de um plano de vida. */
...
/** Meta de inflação do Banco Central: o padrão quando ninguém informa. */
export const INFLACAO_PADRAO_PCT = 4.5;
/** Tabela regressiva: acima de 720 dias, 15%. É o horizonte de um plano. */
export const IR_LONGO_PRAZO_PCT = 15;

/**
 * Converte a rentabilidade real BRUTA na real LÍQUIDA de imposto.
 *
 * A pegadinha que faz todo plano de vida mentir: o Leão cobra sobre o
 * ganho NOMINAL, que inclui a parte que só repôs a inflação. Descontar
 * 15% direto da taxa real subestima muito a mordida.
 *
 * Com 4% real, 4,5% de inflação e 15% de IR, o ganho real líquido cai
 * para cerca de 2,75% — quase um terço a menos, e isso compõe por
 * décadas.
 */
export function rentRealLiquida(
  rentRealPct: number,
  inflacaoPct = INFLACAO_PADRAO_PCT,
  aliquotaIrPct = IR_LONGO_PRAZO_PCT,
): number {
  const real = rentRealPct / 100;
  const inflacao = inflacaoPct / 100;
  const aliquota = aliquotaIrPct / 100;

  const nominal = (1 + real) * (1 + inflacao) - 1;
  const nominalLiquido = nominal * (1 - aliquota);
  return ((1 + nominalLiquido) / (1 + inflacao) - 1) * 100;
}

function simulate(inp: LifePlanInput, opts: { extraMensal?: number; rate?: number; idadeApos?: number } = {}): SimResult {
  // Sempre líquida de IR: projetar com a taxa bruta promete um patrimônio
  // que o cliente não vai ter na conta.
  const i =
    opts.rate ??
    rentRealLiquida(inp.rentRealPct, inp.inflacaoPct, inp.aliquotaIrPct) / 100;
...
export function computeLifePlan(inp: LifePlanInput): LifePlan {
  // Mesma taxa líquida da projeção: o capital necessário na aposentadoria
  // tem de ser calculado com o rendimento que o cliente REALMENTE terá.
  const i =
    rentRealLiquida(inp.rentRealPct, inp.inflacaoPct, inp.aliquotaIrPct) / 100;
